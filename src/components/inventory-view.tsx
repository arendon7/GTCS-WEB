...
export function InventoryView(){
  const {products,stocks,lots,movements,createProduct,resetInventoryDemo}=useInventoryStore();
  const [newName,setNewName]=useState("");
  const [newUnit,setNewUnit]=useState<InventoryUnit>("kg");
  const [feedback,setFeedback]=useState("");

  function addProduct(){
    const result=createProduct(newName,newUnit);
  ...
  return <>
    ...
    <section className="panel mb-4"><div className="section-head"><div><p className="eyebrow">Stock actual</p><h2>Productos disponibles</h2></div><span className="quiet">{stocks.length} combinaciones producto/planta</span></div>
