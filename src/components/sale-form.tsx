...
export function SaleForm(){
  ...
  async function save(){
    ...
    if(!effectivePlantId){setFeedback("Tu sesión no tiene una planta habilitada para registrar ventas.");return;}
  ...
  return <section className="panel mx-auto max-w-3xl"><div className="section-head"><div><p className="eyebrow">Comercial · salida por lote</p><h1 className="text-3xl">Registrar venta</h1><p className="lede">La venta y la salida de inventario están ligadas. El total se deriva de cantidad × precio unitario.</p></div><Link className="button secondary" href="/sales">Cancelar</Link></div>
    {remoteMode&&backend.status==="ready"&&!plantOptions.length&&<div className="mb-5 rounded-xl bg-[var(--surface-soft)] p-4 text-sm text-[var(--muted)]"><strong className="text-[var(--ink)]">Modo solo lectura.</strong> Tu sesión no tiene una planta habilitada para registrar ventas.</div>}
    <div className="grid gap-5 md:grid-cols-2">
      ...
      <label className="grid gap-2 text-xs font-bold text-[var(--muted)] md:col-span-2">Cliente<input className="min-h-11 rounded-lg border border-[var(--line)] px-3 text-sm text-[var(--ink)]" value={customerName} onChange={(event)=>setCustomerName(event.target.value)} placeholder="Nombre o razón social" disabled={submitting||!plantOptions.length} /></label>
      <label className="grid gap-2 text-xs font-bold text-[var(--muted)]">Cantidad vendida {selected?`(${selected.unit})`:""}<input className="min-h-11 rounded-lg border border-[var(--line)] px-3 text-sm text-[var(--ink)]" inputMode="decimal" value={quantity} onChange={(event)=>setQuantity(event.target.value)} placeholder="Ej. 60" disabled={submitting||!plantOptions.length} /></label>
      <label className="grid gap-2 text-xs font-bold text-[var(--muted)]">Precio unitario COP {selected?`/ ${selected.unit}`:""}<input className="min-h-11 rounded-lg border border-[var(--line)] px-3 text-sm text-[var(--ink)]" inputMode="numeric" value={unitPriceCop} onChange={(event)=>setUnitPriceCop(event.target.value)} placeholder="Ej. 2000" disabled={submitting||!plantOptions.length} /></label>
      ...
      <label className="grid gap-2 text-xs font-bold text-[var(--muted)] md:col-span-2">Observación <span className="font-normal">(opcional)</span><textarea className="min-h-24 rounded-lg border border-[var(--line)] p-3 text-sm text-[var(--ink)]" value={note} onChange={(event)=>setNote(event.target.value)} placeholder="Condición comercial o detalle operativo relevante" disabled={submitting||!plantOptions.length} /></label>
    </div>
    {!availableLots.length&&plantOptions.length>0&&<div className="mt-5 rounded-xl border border-dashed border-[var(--line)] p-4 text-xs text-[var(--muted)]">No hay stock disponible en esta planta. <Link className="font-semibold text-[var(--green)]" href="/production/new">Registrar producción</Link>.</div>}
