...
export function ProductionForm(){
  ...
  async function save(){
    if(busy)return;
    if(!effectivePlantId){setFeedback("Tu sesión no tiene una planta habilitada para registrar producción.");return;}
  ...
  return <section className="panel mx-auto max-w-3xl"><div className="section-head"><div><p className="eyebrow">Producto terminado</p><h1 className="text-3xl">Registrar producción</h1><p className="lede">La cantidad se registra como medición independiente. La referencia vigente y el origen quedan congelados en el registro; una pila relacionada nunca copia su peso.</p></div><Link className="button secondary" href="/production">Cancelar</Link></div>
    {remoteMode&&backend.status==="ready"&&!plantOptions.length&&<div className="mb-5 rounded-xl bg-[var(--surface-soft)] p-4 text-sm text-[var(--muted)]"><strong className="text-[var(--ink)]">Modo solo lectura.</strong> Tu sesión no tiene una planta habilitada para registrar producción.</div>}
    <div className="grid gap-5 md:grid-cols-2">
      ...
      <label className="grid gap-2 text-xs font-bold text-[var(--muted)]">Producto<select aria-label="Producto terminado" className="min-h-11 rounded-lg border border-[var(--line)] bg-white px-3 text-sm text-[var(--ink)]" value={effectiveProductId} disabled={busy||!availableProducts.length||!plantOptions.length} onChange={(event)=>setProductId(event.target.value)}>{availableProducts.map((item)=><option value={item.id} key={item.id}>{item.referenceCode?`${item.referenceCode} · `:""}{item.name} · {item.unit}</option>)}</select>{product&&!product.referenceCode&&<span className="font-normal text-[10px]">Este producto aún no tiene referencia canónica; la producción puede registrarse sin inventarla.</span>}</label>
