...
export function ProductionForm(){
  ...
  async function save(){
    if(busy||!plantOptions.length)return;
  ...
  return <section className="panel mx-auto max-w-3xl"><div className="section-head"><div><p className="eyebrow">Producto terminado</p><h1 className="text-3xl">Registrar producción</h1><p className="lede">La cantidad se registra como medición independiente. La referencia vigente y el origen quedan congelados en el registro; una pila relacionada nunca copia su peso.</p></div><Link className="button secondary" href="/production">Cancelar</Link></div>
    {remoteMode&&!plantOptions.length&&<p className="mb-5 rounded-xl bg-[var(--surface-soft)] p-4 text-sm text-[var(--muted)]">Modo solo lectura: esta sesión no tiene una planta autorizada para registrar producción.</p>}
    <div className="grid gap-5 md:grid-cols-2">
      <label className="grid gap-2 text-xs font-bold text-[var(--muted)]">Planta<select className="min-h-11 rounded-lg border border-[var(--line)] bg-white px-3 text-sm text-[var(--ink)]" value={effectivePlantId} disabled={!plantOptions.length||busy} onChange={(event)=>{setPlantId(event.target.value);setSourcePileId("");}}>{plantOptions.map((plant)=><option value={plant.id} key={plant.id}>{plant.name}</option>)}</select></label>
      <label className="grid gap-2 text-xs font-bold text-[var(--muted)]">Producto<select aria-label="Producto terminado" className="min-h-11 rounded-lg border border-[var(--line)] bg-white px-3 text-sm text-[var(--ink)]" value={effectiveProductId} disabled={!availableProducts.length||busy||!plantOptions.length} onChange={(event)=>setProductId(event.target.value)}>{availableProducts.map((item)=><option value={item.id} key={item.id}>{item.referenceCode?`${item.referenceCode} · `:""}{item.name} · {item.unit}</option>)}</select>{product&&!product.referenceCode&&<span className="font-normal text-[10px]">Este producto aún no tiene referencia canónica; la producción puede registrarse sin inventarla.</span>}</label>
      <label className="grid gap-2 text-xs font-bold text-[var(--muted)]">Cantidad producida ({product?.unit ?? "unidad"})<input className="min-h-11 rounded-lg border border-[var(--line)] px-3 text-sm text-[var(--ink)]" inputMode="decimal" value={quantity} onChange={(event)=>setQuantity(event.target.value)} placeholder="Ej. 250" disabled={busy||!plantOptions.length}/></label>
      <label className="grid gap-2 text-xs font-bold text-[var(--muted)]">Proceso fuente<input className="min-h-11 rounded-lg border border-[var(--line)] px-3 text-sm text-[var(--ink)]" value={sourceProcess} onChange={(event)=>setSourceProcess(event.target.value)} placeholder="Ej. Formulación / peletizado" disabled={busy||!plantOptions.length}/></label>
      <label className="grid gap-2 text-xs font-bold text-[var(--muted)] md:col-span-2">Pila cerrada relacionada <span className="font-normal">(opcional)</span><select className="min-h-11 rounded-lg border border-[var(--line)] bg-white px-3 text-sm text-[var(--ink)]" value={sourcePileId} onChange={(event)=>setSourcePileId(event.target.value)} disabled={busy||!plantOptions.length}><option value="">Origen por proceso · sin pila relacionada</option>{closedPiles.map((pile)=><option value={pile.id} key={pile.id}>{pile.code} · {pile.finalWeightKg?.toLocaleString("es-CO")} kg finales</option>)}</select><span className="font-normal text-[10px]">Solo se muestran pilas cerradas de la planta seleccionada. Relacionar una pila crea trazabilidad, no transferencia automática de masa.</span></label>
      <label className="grid gap-2 text-xs font-bold text-[var(--muted)] md:col-span-2">Observación <span className="font-normal">(opcional)</span><textarea className="min-h-24 rounded-lg border border-[var(--line)] p-3 text-sm text-[var(--ink)]" value={note} onChange={(event)=>setNote(event.target.value)} placeholder="Detalle de formulación, empaque o novedad relevante" disabled={busy||!plantOptions.length}/></label>
    </div>
