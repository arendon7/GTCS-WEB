...
export function ProductionForm(){
  ...
  const closedPiles=useMemo(()=>piles.filter((pile)=>pile.plantId===plantId && pile.status==="closed"),[piles,plantId]);

  function save(){
    const result=recordProduction({plantId,productId,quantity:Number(quantity),sourceProcess,sourcePileId:sourcePileId||undefined,note});
  ...
  return <section className="panel mx-auto max-w-3xl"><div className="section-head"><div><p className="eyebrow">Producto terminado</p><h1 className="text-3xl">Registrar producción</h1><p className="lede">La cantidad se registra como medición independiente. Elegir una pila solo crea trazabilidad; no copia su peso final.</p></div><Link className="button secondary" href="/production">Cancelar</Link></div>
    <div className="grid gap-5 md:grid-cols-2">
      <label className="grid gap-2 text-xs font-bold text-[var(--muted)]">Planta<select className="min-h-11 rounded-lg border border-[var(--line)] bg-white px-3 text-sm text-[var(--ink)]" value={plantId} onChange={(event)=>{setPlantId(event.target.value);setSourcePileId("");}}><option value="tamesis">Támesis</option><option value="yarumal">Yarumal</option></select></label>
      <label className="grid gap-2 text-xs font-bold text-[var(--muted)]">Producto<select className="min-h-11 rounded-lg border border-[var(--line)] bg-white px-3 text-sm text-[var(--ink)]" value={productId} onChange={(event)=>setProductId(event.target.value)}>{availableProducts.map((item)=><option value={item.id} key={item.id}>{item.name} · {item.unit}</option>)}</select></label>
    ...
    <div className="mt-6 flex justify-end"><button className="button primary" type="button" onClick={save}>Guardar producción y entrar a inventario</button></div>
  </section>;
}
