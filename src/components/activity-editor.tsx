...
export function ActivityEditor({ activityId, createMode = false }: { activityId?: string; createMode?: boolean }) {
  ...
  if (createMode) {
    ...
    return <section className="panel mx-auto max-w-3xl"><div className="section-head"><div><p className="eyebrow">Actividad no programada</p><h1 className="text-3xl">Registrar e iniciar</h1><p className="lede">Solo para trabajo que no estaba en la programación. Se inicia al guardar.</p></div><Link className="button secondary" href="/app">Cancelar</Link></div><div className="grid gap-5 md:grid-cols-2"><label className="grid gap-2 text-xs font-bold text-[var(--muted)]">Planta<select className="min-h-11 rounded-lg border border-[var(--line)] bg-white px-3 text-sm text-[var(--ink)]" value={effectivePlantId} disabled={plantOptions.length === 0} onChange={(e)=>{setPlantId(e.target.value);setWorkerIds([])}}>{plantOptions.map((plant)=><option key={plant.id} value={plant.id}>{plant.name}</option>)}</select></label><label className="grid gap-2 text-xs font-bold text-[var(--muted)]">Proceso<input className="min-h-11 rounded-lg border border-[var(--line)] px-3 text-sm text-[var(--ink)]" value={process} onChange={(e)=>setProcess(e.target.value)} placeholder="Ej. Compostaje"/></label><label className="grid gap-2 text-xs font-bold text-[var(--muted)] md:col-span-2">Actividad<input className="min-h-11 rounded-lg border border-[var(--line)] px-3 text-sm text-[var(--ink)]" value={title} onChange={(e)=>setTitle(e.target.value)} placeholder="Ej. Limpieza extraordinaria"/></label><label className="grid gap-2 text-xs font-bold text-[var(--muted)] md:col-span-2">Equipo relacionado <span className="font-normal">(opcional)</span><input className="min-h-11 rounded-lg border border-[var(--line)] px-3 text-sm text-[var(--ink)]" value={equipment} onChange={(e)=>setEquipment(e.target.value)} placeholder="Ej. Molino M-01"/></label><div className="md:col-span-2"><WorkerChecks plantId={effectivePlantId} workers={workers} selected={workerIds} onChange={setWorkerIds}/></div></div>{feedback && <p className="mt-4 rounded-lg bg-[var(--red-soft)] p-3 text-sm font-semibold text-[var(--red)]" role="alert">{feedback}</p>}<div className="mt-6 flex justify-end"><button className="button primary" type="button" disabled={busy || plantOptions.length === 0} onClick={submit}>{busy ? "Registrando…" : "Iniciar actividad"}</button></div></section>;
  }
