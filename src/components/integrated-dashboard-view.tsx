...
export function IntegratedDashboardView({initialNowIso}:{initialNowIso:string}){
  ...
  return <>
    ...
    <section className="grid gap-4 xl:grid-cols-2">
      <div className="panel" aria-label="Ventas del periodo"><div className="section-head"><div><p className="eyebrow">Comercial</p><h2>Facturación bruta registrada</h2></div><Link className="text-xs font-semibold text-[var(--green)] underline underline-offset-4" href="/sales">Abrir comercial</Link></div><strong className="text-3xl">{formatMoney(commercialAnalytics.grossBillingCop)}</strong><p className="quiet mt-1">{commercialAnalytics.salesCount} venta{commercialAnalytics.salesCount===1?"":"s"} · no equivale a recaudo ni margen</p><div className="mt-4"><span className="quiet">Cantidad vendida por unidad</span><div className="mt-2"><UnitChips items={commercialAnalytics.soldByUnit}/></div></div>{commercialAnalytics.customers.length?<div className="mt-4 grid gap-2">{commercialAnalytics.customers.slice(0,5).map((item)=><div className="rounded-lg bg-[var(--surface-soft)] p-3 text-xs" key={item.id}><div className="flex items-center justify-between gap-3"><strong>{item.label}</strong><strong>{formatMoney(item.grossCop)}</strong></div><div className="mt-2"><UnitChips items={item.quantities}/></div></div>)}</div>:<p className="quiet mt-4">Sin ventas registradas en este periodo.</p>}</div>
