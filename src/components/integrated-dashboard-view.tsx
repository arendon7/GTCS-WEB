...
function formatMass(kg:number){return kg>=1000?`${(kg/1000).toLocaleString("es-CO",{maximumFractionDigits:2})} t`:`${kg.toLocaleString("es-CO",{maximumFractionDigits:0})} kg`;}
...
export function IntegratedDashboardView({initialNowIso}:{initialNowIso:string}){
  ...
  return <>
    ...
    <section className="panel"><div className="section-head"><div><p className="eyebrow">Eventos</p><h2>Últimos movimientos del periodo</h2></div><span className="quiet">operación + inventario + comercial + gasto</span></div>{combinedEvents.length?<div className="grid gap-2">{combinedEvents.map((event)=><div className="grid gap-2 rounded-lg border border-[var(--line)] p-3 sm:grid-cols-[120px_1fr]" key={event.id}><div><span className="status-pill status-planned">{event.kind}</span><span className="mt-1 block text-[10px] text-[var(--muted)]">{event.plant}</span></div><div><strong className="block text-xs">{event.title}</strong><span className="text-[10px] text-[var(--muted)]">{event.detail} · {new Intl.DateTimeFormat("es-CO",{dateStyle:"medium",timeStyle:"short",timeZone:"America/Bogota"}).format(new Date(event.at))}</span></div></div>)}</div>:<p className="quiet">Sin movimientos en el periodo seleccionado.</p>}</section>
  </>;
}
