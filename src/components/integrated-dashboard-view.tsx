...
export function IntegratedDashboardView(){
  ...
  const csv=useMemo(()=>`${analyticsCsv(analytics)}\n\n${inventoryAnalyticsCsvSection(inventory)}`,[analytics,inventory]);
  const combinedEvents=useMemo(()=>[
    ...analytics.events,
    ...inventory.events,
  ].sort((a,b)=>new Date(b.at).getTime()-new Date(a.at).getTime()).slice(0,40),[analytics.events,inventory.events]);

  return <>
    <header className="page-header"><div><p className="eyebrow">Control operacional</p><h1>Dashboard</h1><p className="lede">Operación, producción e inventario con semántica explícita: flujos por periodo y stock como estado actual.</p></div><div className="header-actions"><Link className="button secondary" href="/">Volver a hoy</Link><ExportCsvButton csv={csv} filename={`greenatics-ops-${analytics.period.startKey}-${analytics.period.endKey}.csv`}/></div></header>
    ...
    <p className="mt-4 text-[11px] text-[var(--muted)]">Producción, movimientos y stock ya provienen de transacciones canónicas. Ventas, ingresos, costos y margen permanecen fuera hasta que exista su registro canónico.</p>
  </>;
}
