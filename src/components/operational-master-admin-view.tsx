...
function SimpleMasterSection({ title, description, kind, items, onChanged }: {
  title: string;
  description: string;
  ...
  async function create() {
    ...
    try {
      const result = await createSimpleOperationalMaster({ kind, plantId: items[0]?.id ? "" : "", code: identity.code, name: identity.name });
      // plantId is injected by the wrapper callback below; this branch is never used directly.
      if (!result.ok) setFeedback({ kind: "error", text: result.error });
    } finally {
      setBusyId("");
    }
  }

  void create; // Creation is rendered by PlantSimpleMasterSection where plantId is known.
  ...
  async function save(item: SimpleItem, active = item.active) {
    ...
    try {
      ...
      await onChanged();
      setFeedback({ kind: "ok", text: "Maestro actualizado." });
    } finally {
      setBusyId("");
    }
  }
  ...
  return <section className="panel">
    ...
    <div className="mt-4 rounded-xl bg-[var(--surface-soft)] p-4"><strong className="text-xs">Crear nuevo</strong><div className="mt-3 grid gap-3 md:grid-cols-[180px_1fr_auto]"><input className="min-h-10 rounded-lg border border-[var(--line)] px-3 text-sm" value={code} onChange={(event) => setCode(normalizeMasterCode(event.target.value))} placeholder="CÓDIGO" aria-label={`Código nuevo ${title}`} /><input className="min-h-10 rounded-lg border border-[var(--line)] px-3 text-sm" value={name} onChange={(event) => setName(event.target.value)} placeholder="Nombre visible" aria-label={`Nombre nuevo ${title}`} /><span className="quiet self-center">Se guarda por planta</span></div></div>
    <FeedbackBox feedback={feedback} />
  </section>;
}

function PlantSimpleMasterSection(props: Omit<React.ComponentProps<typeof SimpleMasterSection>, "onChanged"> & { plantId: string; onChanged: () => Promise<void> }) {
  const { title, description, kind, items, plantId, onChanged } = props;
  const [code, setCode] = useState("");
  const [name, setName] = useState("");
  ...
  async function create() {
    ...
    try {
      ...
      setCode(""); setName("");
      await onChanged();
      setFeedback({ kind: "ok", text: "Registro creado." });
    } finally { setBusy(false); }
  }

  return <div className="grid gap-3">
    <SimpleMasterSection title={title} description={description} kind={kind} items={items} onChanged={onChanged} />
    <div className="panel -mt-3 border-t-0"><div className="grid gap-3 md:grid-cols-[180px_1fr_auto]"><input className="min-h-10 rounded-lg border border-[var(--line)] px-3 text-sm" value={code} onChange={(event) => setCode(normalizeMasterCode(event.target.value))} placeholder="CÓDIGO" aria-label={`Código para crear ${title}`} /><input className="min-h-10 rounded-lg border border-[var(--line)] px-3 text-sm" value={name} onChange={(event) => setName(event.target.value)} placeholder="Nombre visible" aria-label={`Nombre para crear ${title}`} /><button className="button primary" type="button" disabled={busy} onClick={() => void create()}>{busy ? "Creando…" : "Crear"}</button></div><FeedbackBox feedback={feedback} /></div>
  </div>;
}
...
function SourcesSection({ plantId, items, onChanged }: { plantId: string; items: MaterialSource[]; onChanged: () => Promise<void> }) {
  const [code, setCode] = useState("");
  const [name, setName] = useState("");
  const [sourceKind, setSourceKind] = useState<MaterialSourceKind>("generator");
...
function TemplatesSection({ plantId, snapshot, onChanged }: { plantId: string; snapshot: OperationalMasterSnapshot; onChanged: () => Promise<void> }) {
  ...
  async function create() {
    const identity = validateMasterIdentity(code || name, name);
    if (!identity.ok) return setFeedback({ kind: "error", text: identity.error });
    if (!effectiveProcessId) return setFeedback({ kind: "error", text: "Crea o activa al menos un proceso antes de crear plantillas." });
    setBusyId("new"); setFeedback(null);
    try {
...
export function OperationalMasterAdminView() {
  ...
  const load = useCallback(async () => {
    if (backend.mode !== "supabase" || !effectivePlantId) return setSnapshot(emptySnapshot);
  ...
  if (backend.mode !== "supabase") return <section className="panel mx-auto max-w-3xl"><p className="eyebrow">Administración</p><h1 className="text-3xl">Maestros operacionales</h1><p className="lede">La administración canónica de maestros está disponible cuando GREENATICS OPS usa el backend Supabase. El modo local conserva su función de desarrollo y demostración, pero no crea catálogos corporativos.</p></section>;
  if (!managedPlants.length) return <section className="panel mx-auto max-w-3xl"><p className="eyebrow">Administración</p><h1 className="text-3xl">Maestros operacionales</h1><p className="lede">Tu sesión puede operar la planta, pero tu rol no administra catálogos ni programación. Los cambios de maestros requieren Supervisor, Técnico, Administrador o Dirección.</p></section>;
  ...
  return <><header className="page-header"><div><p className="eyebrow">Wave 2A · configuración operacional</p><h1>Maestros operacionales</h1><p className="lede">Una sola taxonomía por planta para que calendario, bitácora, recepción y analítica dejen de depender de texto libre.</p></div><div className="header-actions"><label className="grid gap-1 text-[10px] font-bold uppercase tracking-wide text-[var(--muted)]">Planta<select className="min-h-10 rounded-lg border border-[var(--line)] bg-white px-3 text-sm normal-case text-[var(--ink)]" value={effectivePlantId} onChange={(event) => setPlantId(event.target.value)}>{managedPlants.map((plant) => <option value={plant.plantId} key={plant.plantId}>{plant.name}</option>)}</select></label><button className="button secondary" type="button" disabled={loading} onClick={() => void load()}>{loading ? "Actualizando…" : "Actualizar"}</button></div></header>
    ...
    {loading && !snapshot.processes.length ? <section className="panel"><p className="quiet">Cargando maestros de la planta…</p></section> : <div className="grid gap-4">
      <PlantSimpleMasterSection plantId={effectivePlantId} title="Procesos" description="Eje principal para programación, ejecución y análisis." kind="process" items={snapshot.processes} onChanged={load} />
      <TemplatesSection plantId={effectivePlantId} snapshot={snapshot} onChanged={load} />
      <PlantSimpleMasterSection plantId={effectivePlantId} title="Rutas" description="Circuitos logísticos que después usará Recepción 2.0." kind="route" items={snapshot.routes} onChanged={load} />
      <SourcesSection plantId={effectivePlantId} items={snapshot.sources} onChanged={load} />
      <PlantSimpleMasterSection plantId={effectivePlantId} title="Tipos de material" description="Clasificación canónica del material recibido o utilizado." kind="materialType" items={snapshot.materialTypes} onChanged={load} />
