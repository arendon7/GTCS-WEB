...
export function CompostCreateForm() {
  ...
  const { receptions, access, backend } = useOpsStore();
  ...
  const eligible = useMemo(()=>receptions.filter((reception)=>reception.plantId===effectivePlantId && reception.acceptance!=="rejected"),[receptions,effectivePlantId]);
  ...
  const save = async () => {
    ...
    setFeedback("");
    try {
      const result = await createPile({ plantId: effectivePlantId, location, sourceReceiptIds, initialWeightKg: Number(initialWeightKg) });
