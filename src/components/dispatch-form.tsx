...
export function DispatchForm(){
  ...
  return <section className="panel mx-auto max-w-3xl"><div className="section-head"><div><p className="eyebrow">Kardex · salida</p><h1 className="text-3xl">Registrar despacho / salida</h1><p className="lede">La salida se descuenta de un lote específico. El sistema bloquea cualquier movimiento que deje stock negativo.</p></div><Link className="button secondary" href="/inventory">Cancelar</Link></div>
    {remoteMode&&!plantOptions.length&&<p className="mb-5 rounded-xl bg-[var(--surface-soft)] p-4 text-sm text-[var(--muted)]">Modo solo lectura: esta sesión no tiene una planta autorizada para registrar salidas de inventario.</p>}
    <div className="grid gap-5 md:grid-cols-2">
      ...
      <label className="grid min-w-0 gap-2 text-xs font-bold text-[var(--muted)]">Lote con stock<select aria-label="Lote con stock" className="min-h-11 w-full min-w-0 max-w-full rounded-lg border border-[var(--line)] bg-white px-3 text-sm text-[var(--ink)]" value={selected?`${selected.productId}|${selected.lotCode}`:""} onChange={(event)=>setLotKey(event.target.value)} disabled={!availableLots.length||busy||!plantOptions.length}><option value="">Seleccionar lote</option>{availableLots.map((lot)=><option value={`${lot.productId}|${lot.lotCode}`} key={`${lot.productId}-${lot.lotCode}`}>{lot.lotCode} · {lot.productName} · {lot.quantity.toLocaleString("es-CO")} {lot.unit}</option>)}</select></label>
    ...
    {!availableLots.length&&plantOptions.length&&<div className="mt-5 rounded-xl border border-dashed border-[var(--line)] p-4 text-xs text-[var(--muted)]">No hay stock en esta planta. <Link className="font-semibold text-[var(--green)]" href="/production/new">Registrar producción</Link>.</div>}
    {feedback&&<p role="alert" className="mt-4 rounded-lg bg-[var(--red-soft)] p-3 text-sm font-semibold text-[var(--red)]">{feedback}</p>}
    <div className="mt-6 flex justify-end"><button className="button primary" type="button" onClick={save} disabled={busy||!plantOptions.length||!availableLots.length}>{busy?"Registrando salida…":"Registrar salida"}</button></div>
  </section>;
}
