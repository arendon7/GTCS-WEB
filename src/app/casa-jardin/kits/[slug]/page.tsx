...
export default async function CasaJardinKitDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <>
      ...
      <section className="homegarden-detail-route" id="ruta-kit" aria-labelledby="homegarden-kit-route-title"><div className="container"><div className="homegarden-detail-route__heading"><div><span className="eyebrow">Cómo usar el kit</span><h2 id="homegarden-kit-route-title">Una composición completa no significa aplicar todo al mismo tiempo.</h2></div><p>El kit es una forma sencilla de tener una ruta disponible. La planta, su etapa y su condición indican qué referencia entra primero y cuál puede esperar.</p></div><ol><li><span>01</span><div><strong>Contexto</strong><p>Define si estás cuidando follaje, flores, huerta, colección o una planta recién trasplantada.</p></div></li><li><span>02</span><div><strong>Etapa</strong><p>Elige la referencia que corresponde al momento observable; no uses todas por calendario.</p></div></li><li><span>03</span><div><strong>Condición</strong><p>Si hay estrés, encharcamiento o daño sanitario, pausa y revisa la causa antes de nutrir.</p></div></li><li><span>04</span><div><strong>Seguimiento</strong><p>Registra fecha, cantidad aplicada y respuesta para decidir el siguiente paso con más información.</p></div></li></ol><div className="button-row"><Link className="button button--dark" href="/casa-jardin/#diagnostico">Usar orientador</Link><Link className="button button--ghost" href="/casa-jardin/guias/">Abrir guías</Link><Link className="button button--ghost" href="/contacto/?interes=casa-jardin">Consultar disponibilidad</Link></div></div></section>
