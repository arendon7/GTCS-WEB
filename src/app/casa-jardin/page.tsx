...
const kitCards = [
  ...
  },
  {
    ...
    image: "/kits/kit-casa-completa-v1.png",
...
export default function CasaJardinPage() {
  return (
    <>
      ...
      <section className="homegarden-v4-kits" aria-labelledby="homegarden-kits-title"><div className="container"><div className="wg-v4-heading"><div><span className="eyebrow">Arquitectura de prelanzamiento</span><h2 id="homegarden-kits-title">Kits pensados para empezar por una necesidad real.</h2></div><p>Las combinaciones conservan la arquitectura V1 de Casa & Jardín: orientan la conversación y el armado del pedido, pero no sustituyen la validación de presentación, disponibilidad o recomendación técnica. Cada kit tiene una lógica de uso; los cuatro productos no se aplican al mismo tiempo. Las imágenes corresponden al handoff visual de prelanzamiento.</p></div><div className="homegarden-v4-kits__grid">{kitCards.map(({ title, composition, intent, claim, image, alt }, index) => <article className="homegarden-v4-kits__card homegarden-v4-kits__card--visual" key={title}><figure className="homegarden-v4-kits__visual"><Image src={image} alt={alt} fill sizes="(max-width: 760px) 100vw, 20vw" /><figcaption>Arte de prelanzamiento</figcaption></figure><div className="homegarden-v4-kits__body"><span>0{index + 1} · Prelanzamiento</span><h3>{title}</h3><p>{composition}</p><p className="homegarden-v4-kits__claim">{claim}</p><p className="homegarden-v4-kits__intent">{intent}</p><small>Sin precio público ni checkout habilitado.</small></div></article>)}</div><div className="homegarden-v4-kits__note"><strong>Trasplanta & Arranca</strong><p>Este kit permanece bloqueado hasta cerrar la verdad de producto del componente radicular o biológico. No lo presentamos como disponible.</p></div></div></section>
