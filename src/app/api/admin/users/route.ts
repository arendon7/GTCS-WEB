...
type OwnMembershipRow={plant_id:string;role:"admin"|"director";plants:{id:string;code:string;name:string;active:boolean}|Array<{id:string;code:string;name:string;active:boolean}>|null};

function noStoreJson(body:unknown,status=200){const response=NextResponse.json(body,{status});response.headers.set("Cache-Control","private, no-store");return response;}
function onePlant(value:OwnMembershipRow["plants"]){return Array.isArray(value)?value[0]:value;}
...
async function adminContext(){
  ...
  const {data,error}=await supabase.from("plant_memberships").select("plant_id,role,plants!inner(id,code,name,active)").eq("user_id",user.id).eq("active",true).in("role",["admin","director"]);
  if(error)return {ok:false as const,status:500,error:"No fue posible validar permisos administrativos."};
  const managedPlants=((data??[]) as unknown as OwnMembershipRow[]).flatMap((row)=>{const plant=onePlant(row.plants);if(!plant||!plant.active)return [];return [{plantId:plant.id,code:plant.code,name:plant.name,managerRole:row.role} satisfies ManagedPlant];});
  if(!managedPlants.length)return {ok:false as const,status:403,error:"No tienes plantas administrables."};
  return {ok:true as const,supabase,user,managedPlants};
}
...
function authorizeAssignments(assignments:UserMembershipAssignment[],managedPlants:ManagedPlant[]){
  ...
  for(const assignment of assignments){const plant=byId.get(assignment.plantId);if(!plant)return `No administras la planta ${assignment.plantId}.`;if(assignment.role==="director"&&plant.managerRole!=="director")return `Solo un director puede asignar otro director en ${plant.name}.`;}
  return undefined;
}

export async function GET(){
  const context=await adminContext();if(!context.ok)return noStoreJson({ok:false,error:context.error},context.status);
  ...
  for(const row of memberships){const current=users.get(row.user_id)??{id:row.user_id,email:emailById.get(row.user_id)??"",displayName:row.display_name,memberships:[]};current.memberships.push({plantId:row.plant_id,plantName:row.plant_name,role:row.role,active:row.active});users.set(row.user_id,current);}
  return noStoreJson({ok:true,managedPlants:context.managedPlants,users:[...users.values()]});
}

export async function POST(request:NextRequest){
  const context=await adminContext();if(!context.ok)return noStoreJson({ok:false,error:context.error},context.status);
  let raw:unknown;try{raw=await request.json();}catch{return noStoreJson({ok:false,error:"JSON inválido."},400);}
  const parsed=validateInviteUserInput(raw);if(!parsed.ok)return noStoreJson(parsed,400);
  const authorizationError=authorizeAssignments(parsed.value.assignments,context.managedPlants);if(authorizationError)return noStoreJson({ok:false,error:authorizationError},403);
  ...
  if(membershipError){await admin.auth.admin.deleteUser(invitedUserId);return noStoreJson({ok:false,error:"La invitación se revirtió porque no fue posible asignar las membresías."},500);}
...
export async function PATCH(request:NextRequest){
  const context=await adminContext();if(!context.ok)return noStoreJson({ok:false,error:context.error},context.status);
  let raw:unknown;try{raw=await request.json();}catch{return noStoreJson({ok:false,error:"JSON inválido."},400);}
  const parsed=validateUpdateUserInput(raw);if(!parsed.ok)return noStoreJson(parsed,400);
  const authorizationError=authorizeAssignments(parsed.value.assignments,context.managedPlants);if(authorizationError)return noStoreJson({ok:false,error:authorizationError},403);
  const {error}=await context.supabase.rpc("admin_set_user_memberships",{target_user:parsed.value.userId,target_display_name:parsed.value.displayName,assignments:parsed.value.assignments});
