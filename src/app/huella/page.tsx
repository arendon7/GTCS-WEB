...
const huellaAccessLabel = runtimeLinks.huella ? "Entrar a la plataforma" : "Configurar la plataforma";
...
const huellaModules = [
  ["Inventarios", "Periodos, sedes, alcances y responsables organizados en un espacio de trabajo."],
  ["Fuentes", "Consumos, residuos, transporte, energía y actividad con origen, unidad y periodo."],
  ["Cargas", "Importación de CSV/XLSX, mapeo de columnas y revisión de cada lote antes de aplicarlo."],
  ["Calidad del dato", "Duplicados, faltantes, unidades inconsistentes y decisiones pendientes visibles."],
  ["Evidencia", "Soportes, notas, correcciones, autor, fecha y versión vinculados al registro."],
  ["Reportes", "Resultados con método, frontera, supuestos, estado de revisión y contexto para comunicar."],
] as const;
  ...
  return (
    <>
      ...
      <ToolAccessRail
        ...
        status={runtimeLinks.huella ? "Plataforma de inventarios disponible" : "Estimador público disponible"}
      ...
      <section className="carbon-v4-product-deck" id="plataforma" aria-labelledby="huella-product-title">
        <div className="container">
          <div className="digital-v4-heading">
            <div><span className="eyebrow">La herramienta completa</span><h2 id="huella-product-title">No es solo una calculadora: es un sistema para construir un inventario defendible.</h2></div>
            <p>La experiencia completa acompaña el trabajo desde el primer archivo hasta el resultado revisado. Cada módulo responde una pregunta concreta: qué se midió, de dónde viene, qué falta, quién lo revisó y qué puede comunicarse.</p>
          </div>
          <div className="carbon-v4-product-deck__layout">
            <div className="carbon-v4-product-deck__modules">
              {huellaModules.map(([title, copy], index) => <article key={title}><span>{String(index + 1).padStart(2, "0")}</span><div><strong>{title}</strong><p>{copy}</p></div></article>)}
            </div>
            <div className="carbon-v4-product-deck__screens">
              <figure><div><Image src="/tools/huella/dashboard.png" alt="Dashboard de Calcula tu Huella con avance y alertas del inventario" fill sizes="(max-width: 900px) 100vw, 38vw" /></div><figcaption><strong>Centro de trabajo</strong><span>Avance, alertas y próxima acción.</span></figcaption></figure>
              <figure><div><Image src="/tools/huella/recorrido.png" alt="Recorrido por etapas de Calcula tu Huella" fill sizes="(max-width: 900px) 100vw, 38vw" /></div><figcaption><strong>Recorrido guiado</strong><span>Del dato inicial al reporte.</span></figcaption></figure>
            </div>
          </div>
          <div className="carbon-v4-product-deck__note"><strong>Para quién está pensado:</strong><span>responsables de sostenibilidad, equipos operativos, consultores, dirección y revisores que necesitan trabajar sobre la misma fuente sin perder el contexto.</span></div>
        </div>
      </section>
