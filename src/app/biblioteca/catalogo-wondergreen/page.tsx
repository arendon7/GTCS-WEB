...
const decisionSteps = [
  ...
  ["05", "Programa técnico", "Integra ficha, etiqueta, presentación, condición de uso, cotización y recomendación antes de comprar o aplicar."],
] as const;
...
const outsideCatalog = [
  ...
  ["Guía de aplicación", "Organiza la preparación, la ejecución y el seguimiento; la ficha técnica de cada referencia define dosis y compatibilidades."],
...
const productRows = products.map((product) => ({
  ...
  status: typeof product.priceCop === "number" ? "Precio público" : "Bajo cotización",
...
export default function WondergreenCatalogGuidePage() {
  ...
  return (
    <>
      <ArticleJsonLd headline="Catálogo técnico-comercial Wondergreen: cómo leer el sistema" description="Arquitectura del portafolio Wondergreen por objetivo agronómico, etapa, formato y programa técnico." url={url} dateModified="2026-08-11" about={["Wondergreen Nutrients", "Nutrición vegetal", "Fertilizantes organominerales", "Bioinsumos"]} />
      ...
      <section className="knowledge-hero knowledge-hero--wondergreen"><div className="container knowledge-hero-grid"><div><Link className="back-link" href="/biblioteca/">← Biblioteca</Link><span className="eyebrow">Catálogo técnico-comercial</span><div className="knowledge-wg-logo"><img src="/brand/wondergreen-nutrients.webp" alt="Wondergreen Nutrients" width="420" height="221" /></div><h1>El portafolio se entiende mejor cuando se lee como sistema.</h1><p className="lead">Wondergreen integra acondicionadores de suelo, fertilizantes organominerales sólidos y líquidos y bioinsumos para manejo integrado. La selección empieza por el objetivo y se convierte en programa con una referencia, una presentación y un protocolo de uso claros.</p></div><aside className="knowledge-warning knowledge-warning--light"><span>Idea central</span><strong>Precio público, cotización y programa técnico cumplen funciones diferentes.</strong><p>Las referencias con precio público facilitan la consulta inmediata; las líneas especializadas se suministran bajo cotización y acompañamiento técnico. En ambos casos, el programa define la mejor aplicación.</p></aside></div></section>
      ...
      <section className="knowledge-section"><div className="container"><div className="section-heading"><span className="eyebrow">Product Master público</span><h2>Referencias, formatos y presentaciones documentadas.</h2><p>El estado “precio público” habilita una referencia comercial en la web; “bajo cotización” activa acompañamiento técnico para referencias especializadas, volúmenes y programas por cultivo.</p></div><div className="portfolio-table-wrap" role="region" aria-label="Product Master Wondergreen; desplázate horizontalmente para ver todas las columnas" tabIndex={0}><table className="portfolio-table portfolio-table--catalog"><thead><tr><th>Familia</th><th>Referencia</th><th>Categoría</th><th>Formato</th><th>Fórmula/ref.</th><th>Presentaciones</th><th>Estado</th></tr></thead><tbody>{productRows.map((row)=><tr key={row.name}><td>{row.family}</td><td><Link href={row.href}><strong>{row.name}</strong></Link></td><td>{row.category}</td><td>{row.format}</td><td>{row.formula}</td><td>{row.presentations}</td><td><span className={row.status === "Precio público" ? "catalog-status catalog-status--priced" : "catalog-status catalog-status--technical"}>{row.status}</span></td></tr>)}</tbody></table></div><div className="catalog-summary-grid"><article><strong>{pricedProducts.length}</strong><span>referencias con precio público de referencia</span></article><article><strong>{technicalPortfolio.length}</strong><span>referencias disponibles bajo cotización</span></article><article><strong>{bioinputs.length}</strong><span>bioinsumos integrados al portafolio técnico</span></article></div></div></section>

      <section className="knowledge-section knowledge-section--soft"><div className="container"><div className="section-heading"><span className="eyebrow">Bioinsumos</span><h2>Biología integrada a programas de manejo.</h2><p>El Product Master reúne extractos botánicos y microorganismos. La ficha técnica de cada presentación define cepas, concentraciones, dosis, compatibilidades, objetivo de manejo y condición de uso para construir una aplicación responsable.</p></div><div className="knowledge-card-grid">{bioinputs.map((product)=><article key={product.slug}><span className="knowledge-card-kicker">{product.format} · {product.presentations.join(" / ")}</span><h3>{product.name}</h3><p>{product.technicalRole}</p><Link className="knowledge-inline-link" href={`/wondergreen/productos/${product.slug}/`}>Ver ficha técnica →</Link></article>)}</div></div></section>
