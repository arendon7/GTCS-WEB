    ...
    <>
      ...
      <section className="crop-detail-hero">
        <div className="container crop-detail-grid">
          <div><Link className="back-link" href="/wondergreen/cultivos/">← Todos los cultivos</Link><span className="eyebrow">Wondergreen · {crop.name}</span><h1>{crop.headline}</h1><p className="lead">{crop.intro}</p></div>
          <aside className="crop-context"><span>Antes de recomendar</span><p>{crop.context}</p></aside>
        </div>
      </section>

      <section className="crop-field-check">
        <div className="container"><div className="section-heading"><span className="eyebrow">Checklist de campo</span><h2>Seis preguntas antes de elegir una línea.</h2></div><div className="crop-check-grid">{fieldChecklist.map((item,index)=><article key={item}><span>{String(index+1).padStart(2,"0")}</span><p>{item}</p></article>)}</div></div>
      </section>

      <section className="crop-program">
        <div className="container"><div className="section-heading"><span className="eyebrow">Programa por momento</span><h2>Lee la necesidad antes de elegir el producto.</h2></div><div className="crop-stage-list">{crop.stages.map((stage,index)=><article key={stage.moment}><span className="crop-stage-number">{String(index+1).padStart(2,"0")}</span><div><strong>{stage.moment}</strong><p>{stage.goal}</p></div><div className="line-pills">{stage.lines.map((line)=><span key={line}>{line}</span>)}</div></article>)}</div></div>
      </section>

      <section className="crop-application-section">
        <div className="container crop-application-grid"><div><span className="eyebrow">Uso en campo</span><h2>Una recomendación debe ser fácil de ejecutar correctamente.</h2><p>Estas reglas son generales. La dosis, concentración, compatibilidad y vía definitiva dependen de la ficha vigente y del contexto del lote.</p></div><ol>{fieldApplicationRules.map((rule)=><li key={rule}>{rule}</li>)}</ol></div>
      </section>

      <section className="crop-alert-section">
        <div className="container crop-alert-grid"><div><span className="eyebrow eyebrow--light">Señales de alerta</span><h2>No todo síntoma se resuelve agregando nutrición.</h2></div><div>{crop.alerts.map((alert)=><p key={alert}>{alert}</p>)}</div></div>
      </section>

      <section className="crop-followup-section">
        <div className="container"><div className="section-heading"><span className="eyebrow">Después de aplicar</span><h2>La siguiente recomendación empieza con el seguimiento.</h2></div><div className="crop-followup-grid">{crop.followUp.map((item,index)=><article key={item}><span>{String(index+1).padStart(2,"0")}</span><p>{item}</p></article>)}</div></div>
      </section>
