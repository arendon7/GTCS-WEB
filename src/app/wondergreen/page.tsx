...
export default function WondergreenPage() {
  return (
    <>
      <section className="wg-v4-hero">
        <div className="container wg-v4-hero__grid">
          <div className="wg-v4-hero__copy">
            ...
            <p className="lead">Wondergreen organiza compost, fertilizantes organominerales y bioinsumos por objetivo y etapa. La selección se convierte en programa cuando incorpora diagnóstico, etiqueta vigente, aplicación, observación y ajuste.</p>
      ...
      <section className="wg-v4-tools"><div className="container wg-v4-tools__grid"><div><span className="eyebrow eyebrow--light">Herramientas para preparar la conversación</span><h2>Llega a la recomendación con mejores preguntas y mejores datos.</h2><p>Las herramientas producen orientaciones y escenarios. El programa técnico integra además contexto de campo, criterio agronómico y seguimiento.</p><Link className="button button--light" href="/recursos/">Ver todas las herramientas</Link></div><div className="wg-v4-tools__list">{tools.map(([title, copy, href], index) => <Link href={href} key={title}><span>0{index + 1}</span><div><strong>{title}</strong><p>{copy}</p></div><em>→</em></Link>)}</div></div></section>
