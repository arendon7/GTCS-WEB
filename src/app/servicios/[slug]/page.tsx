...
const categoryProcess = {
  ...
  Valorización: [
    ["01", "Caracterizar la oportunidad", "Leemos la biomasa, el contexto, la escala y el resultado productivo o energético esperado."],
    ["02", "Elegir la ruta", "Comparamos procesos, productos, usos, calidad requerida, infraestructura y condiciones de mercado."],
    ["03", "Diseñar y validar", "Definimos formulación, proceso, protocolo de uso y pruebas necesarias antes de escalar."],
    ["04", "Implementar", "Articulamos tecnología, operación, producto, acompañamiento y registro según el alcance."],
    ["05", "Medir y mejorar", "Seguimos desempeño, calidad, uso, impacto y oportunidades de continuidad o expansión."],
  ],
} as const;
    ...
    <>
      ...
      <section className="service-detail-context">
        <div className="container">
          <div className="service-detail-context__heading">
            <div><span className="eyebrow">La decisión detrás del servicio</span><h2>{service.headline}</h2></div>
            <p>{service.intro}</p>
          </div>
          <div className="service-detail-context__grid">
            <article><span>Cuando aparece este problema</span><p>{service.challenge}</p></article>
            <article><span>Cómo lo abordamos</span><p>{service.solution}</p></article>
          </div>
        </div>
      </section>
      ...
      <section className="service-detail-practice">
        <div className="container service-detail-practice__grid">
          <div>
            <span className="eyebrow">En la práctica</span>
            <h2>El alcance se vuelve trabajo observable.</h2>
            <p>Estas son actividades posibles dentro de una implementación. Se ajustan a la corriente, la escala, el equipo disponible y las responsabilidades acordadas.</p>
            <dl className="service-detail-facts">
              <div><dt>Ámbito habitual</dt><dd>{service.scope}</dd></div>
              <div><dt>Experiencia relacionada</dt><dd>{service.cases.join(" · ")}</dd></div>
            </dl>
          </div>
          <ol className="service-detail-activities">
            {service.activities.map((activity, index) => <li key={activity}><span>{String(index + 1).padStart(2, "0")}</span><strong>{activity}</strong></li>)}
          </ol>
        </div>
      </section>
