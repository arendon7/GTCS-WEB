...
test("company page explains Greenatics as a connected system", async ({ page }) => {
  ...
  await expect(page.getByRole("link", { name: "Wondergreen" })).toHaveAttribute("href", "/wondergreen");
  await expect(page.getByRole("link", { name: "Impacto" })).toHaveAttribute("href", "/impacto");
});
