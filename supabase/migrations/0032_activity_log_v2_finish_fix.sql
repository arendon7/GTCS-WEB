-- Wave 2B.1 hotfix · keep the public RPC signature stable while removing
-- the PL/pgSQL parameter/column ambiguity introduced by activity_comment.
...
declare
  ...
  effective_unit text;
  effective_activity_comment text := nullif(btrim(activity_comment),'');
begin
  ...
  set ended_at=finished_at,
      ...
      activity_comment=effective_activity_comment
...
-- CREATE OR REPLACE preserves the existing function identity and grants. Keep the
-- execution boundary explicit for fresh and already-migrated environments alike.
...
comment on function public.ops_finish_activity_v2(uuid,numeric,text,text,text,boolean,text,uuid[]) is
  'Closes a running activity while recording result, final operational comment, tools used and optional novelty/incident.';
