...
insert into public.employees(id,plant_id,display_name,active) values
 ('a7000000-0000-4000-8000-000000000001','a2000000-0000-4000-8000-000000000001','Maintenance QA Worker',true);
insert into public.operational_processes(id,plant_id,code,name,active) values
 ('a8000000-0000-4000-8000-000000000001','a2000000-0000-4000-8000-000000000001','MANTENIMIENTO','Mantenimiento',true);
insert into public.activity_templates(id,plant_id,process_id,code,name,allows_unplanned,active) values
 ('a9000000-0000-4000-8000-000000000001','a2000000-0000-4000-8000-000000000001','a8000000-0000-4000-8000-000000000001','MANTENIMIENTO_HERRAMIENTAS_EQUIPOS','Mantenimiento de herramientas o equipos',true,true);
...
select throws_like($$select public.ops_close_equipment_repair_v2((select id from public.maintenance_tickets where equipment_id='a3000000-0000-4000-8000-000000000001'),now()-interval '1 hour','Rodamiento fatigado','Cambio de rodamiento',array['a7000000-0000-4000-8000-000000000001'::uuid],array['a5000000-0000-4000-8000-000000000001'::uuid],array['LOT-MNT-QA'],array[6::numeric],array[]::text[])$$,'%Stock insuficiente%','close rolls back when spare stock is insufficient');
...
select lives_ok($$select public.ops_close_equipment_repair_v2((select id from public.maintenance_tickets where equipment_id='a3000000-0000-4000-8000-000000000001'),now()-interval '1 hour','Rodamiento fatigado','Cambio de rodamiento y prueba funcional',array['a7000000-0000-4000-8000-000000000001'::uuid],array['a5000000-0000-4000-8000-000000000001'::uuid],array['LOT-MNT-QA'],array[2::numeric],array['evidencia://reparacion-qa'])$$,'repair closes atomically with physical spare consumption');
