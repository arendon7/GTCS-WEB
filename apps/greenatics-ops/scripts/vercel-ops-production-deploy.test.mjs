...
describe("Vercel stable OPS production deployment", () => {
  it("publishes the exact full-ops SHA, keeps full-ops env on preview+production and assigns the stable alias", async () => {
    const calls = [];
    ...
    expect(envCall.body.every((item) => JSON.stringify(item.target) === JSON.stringify(["production", "preview"]))).toBe(true);
