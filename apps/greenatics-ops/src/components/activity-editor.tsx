...
export function ActivityEditor({ activityId, createMode = false }: { activityId?: string; createMode?: boolean }) {
  ...
  const plantOptions = useMemo(() => backend.mode === "supabase" ? access.map((plant) => ({ id: plant.plantId, name: plant.name, dbId: plant.dbId })) : [{ id: "tamesis", name: "Támesis", dbId: "" }, { id: "yarumal", name: "Yarumal", dbId: "" }], [access, backend.mode]);
  ...
  if (createMode) {
    ...
    </div>}{feedback ? <p role="alert" className="mt-4 rounded-lg bg-[var(--red-soft)] p-3 text-sm font-semibold text-[var(--red)]">{feedback}</p> : null}<div className="mt-6 flex justify-end"><button className="button primary" disabled={busy || !effectivePlant} onClick={() => void submit()}>{busy ? "Guardando…" : backend.mode !== "supabase" ? "Iniciar actividad" : completed ? "Registrar actividad finalizada" : "Registrar e iniciar"}</button></div></section>;
  }

  if (!activity) return null;
  const workerNames = activity.workerIds.map((id) => workers.find((worker) => worker.id === id)?.name).filter(Boolean).join(" + ");
  async function start() { setBusy(true); try { const result = await startActivity(activity!.id, workerIds); if (!result.ok) return setFeedback(result.error); if (result.id !== activity!.id) router.replace(`/activities/${result.id}`); else setFeedback("Actividad iniciada correctamente."); } finally { setBusy(false); } }
  async function finish() {
    setBusy(true); setFeedback("");
    try {
      const parsed = quantity.trim() ? Number(quantity) : undefined;
      if (backend.mode === "supabase") { await finishRemoteActivity(activity!.id, { quantity: parsed, unit: parsed ? effectiveUnit : undefined, noveltyType: noveltyType || undefined, novelty: novelty.trim() || undefined, openIncident, comment: comment.trim() || undefined, toolIds }); await refresh(); router.push("/app"); return; }
