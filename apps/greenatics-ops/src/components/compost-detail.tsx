...
export function CompostDetail({ pileId }: { pileId: string }) {
  ...
  return <>
    <header className="page-header"><div><p className="eyebrow">{pile.plant} · {pile.location}</p><h1>{pile.code}</h1><p className="lede">Trazabilidad física, eventos operacionales, bitácora canónica, controles técnicos y rendimiento.</p></div><div className="header-actions"><Link className="button secondary" href="/compost">Volver</Link><CompostStatusPill status={pile.status} /></div></header>
    ...
    {pile.status !== "closed" && <section className="panel mx-auto mt-4 max-w-5xl">
      ...
      <p className="mt-4 rounded-lg bg-[var(--surface-soft)] p-3 text-xs text-[var(--muted)]">Al guardar, el evento también se registra una sola vez en la Bitácora operacional con estos trabajadores y horarios; no debes duplicar la actividad.</p>
    ...
    <section className="panel mx-auto mt-4 max-w-5xl"><div className="section-head"><div><p className="eyebrow">Eventos</p><h2>Histórico operacional</h2></div><span className="quiet">{pileEvents.length} eventos</span></div><div className="grid gap-3">{pileEvents.length ? pileEvents.map((event) => <article className="rounded-xl border border-[var(--line)] p-4" key={event.id}><div className="flex flex-wrap items-center justify-between gap-3"><div className="flex flex-wrap items-center gap-2"><strong className="text-sm">{eventLabels[event.type]}</strong><span className={`rounded-full px-2 py-1 text-[10px] font-extrabold ${event.activityId ? "bg-[var(--green-soft)] text-[var(--green-dark)]" : "bg-[var(--surface-soft)] text-[var(--muted)]"}`}>{event.activityId ? "Bitácora enlazada" : "Evento previo sin vínculo canónico"}</span></div><span className="text-xs text-[var(--muted)]">{bogotaTime.format(new Date(event.startedAt))}–{bogotaTime.format(new Date(event.endedAt))}</span></div><div className="mt-2 flex flex-wrap gap-3 text-xs text-[var(--muted)]"><span>{(compostEventDurationHours(event) * 60).toFixed(0)} min</span>{event.volumeM3 !== undefined && <span>{event.volumeM3.toFixed(2)} m³</span>}<span>{event.workerIds.length} trabajador(es)</span>{compostEventProductivity(event) > 0 && <span>{compostEventProductivity(event).toFixed(2)} m³/trab·h</span>}</div><div className="mt-2 text-xs text-[var(--muted)]">{event.workerIds.map((id) => workerNames.get(id) ?? "Trabajador histórico").join(" · ")}</div>{event.notes && <p className="mt-2 text-xs text-[var(--muted)]">{event.notes}</p>}</article>) : <p className="text-xs text-[var(--muted)]">Las pilas creadas antes de Compostaje 2.0 pueden no tener eventos operacionales estructurados.</p>}</div></section>
