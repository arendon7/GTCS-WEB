...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  const canManageReadiness = backend.mode === "supabase" && access.some((plant) => canManageHomeGardenReadiness(plant.role));
  ...
  return <div className="app-shell"><aside className="sidebar" aria-label="Navegación principal"><a className="brand-block" href={greenaticsPublicUrl} aria-label="Volver a Greenatics"><div className="brand-mark" aria-hidden="true">G</div><div><strong>GREENATICS</strong><span>OPS</span></div></a><nav className="side-nav">{nav.map(([label, href]) => <Link key={label} href={href}>{label}</Link>)}{canManageReadiness ? <Link href="/admin/wondergreen-readiness">Readiness Wondergreen</Link> : null}{canManageOperations ? <Link href="/admin/operations">Maestros operacionales</Link> : null}{canManageUsers ? <Link href="/admin/leads">Consultas comerciales</Link> : null}{canManageUsers ? <Link href="/admin/users">Usuarios y accesos</Link> : null}</nav><div className="sidebar-foot" title={backend.error}><div><span className={`status-dot ${backend.status === "ready" && modulesReady && !remoteError ? "status-ok" : ""}`} /> {sourceLabel}</div><a className="mt-2 block text-[11px] font-semibold underline" href={greenaticsPublicUrl}>Volver al sitio Greenatics</a>{backend.mode === "supabase" && identity ? <div className="mt-2"><strong className="block text-[11px]">{identity.displayName}</strong><button className="mt-1 text-[11px] underline" type="button" disabled={signingOut} onClick={closeSession}>{signingOut ? "Cerrando…" : "Cerrar sesión"}</button></div> : null}{backend.mode === "supabase" && backend.status === "error" ? <Link className="mt-2 block text-[11px] underline" href="/login">Ir a acceso</Link> : null}</div></aside><main className="main-area">
