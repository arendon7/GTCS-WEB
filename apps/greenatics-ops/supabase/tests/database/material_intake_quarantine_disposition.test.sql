...
reset role;
select set_config('test.tam_lot_id',(select id::text from public.material_intake_lots where plant_id='f2000000-0000-4000-8000-000000000001'),true);
...
reset role;
select set_config('test.yar_lot_id',(select id::text from public.material_intake_lots where plant_id='f2000000-0000-4000-8000-000000000002'),true);

set local role authenticated;
set local request.jwt.claim.sub='f1000000-0000-4000-8000-000000000002';
select throws_like(
  $$select public.ops_dispose_material_intake_lot(current_setting('test.tam_lot_id')::uuid,'release','Control conforme')$$,
...
set local request.jwt.claim.sub='f1000000-0000-4000-8000-000000000001';
select lives_ok(
  $$select public.ops_dispose_material_intake_lot(current_setting('test.tam_lot_id')::uuid,'release','Control de calidad conforme; lote apto para proceso')$$,
  'technical user releases TAM quarantine'
);
select is((select status from public.material_intake_lots where id=current_setting('test.tam_lot_id')::uuid),'available'::text,'released lot becomes available');
...
select throws_like(
  $$select public.ops_dispose_material_intake_lot(current_setting('test.tam_lot_id')::uuid,'reject','Cambio de criterio')$$,
  '%Solo un lote en cuarentena%',
  'resolved lot cannot be disposed twice'
);
select throws_like(
  $$select public.ops_dispose_material_intake_lot(current_setting('test.yar_lot_id')::uuid,'release','Intento cruzado')$$,
  '%No tienes permiso para resolver cuarentenas%',
  'technical user cannot resolve another plant quarantine'
);
select throws_like(
  $$select public.ops_dispose_material_intake_lot(current_setting('test.yar_lot_id')::uuid,'reject','   ')$$,
  '%Registra el motivo%',
...
set local request.jwt.claim.sub='f1000000-0000-4000-8000-000000000003';
select lives_ok(
  $$select public.ops_dispose_material_intake_lot(current_setting('test.yar_lot_id')::uuid,'reject','Contaminación no conforme; salida del flujo de aprovechamiento')$$,
  'technical user finally rejects YAR quarantine'
);
select is((select status from public.material_intake_lots where id=current_setting('test.yar_lot_id')::uuid),'rejected'::text,'finally rejected lot receives explicit terminal status');
select is((select available_mass_kg from public.material_intake_lots where id=current_setting('test.yar_lot_id')::uuid),750::numeric,'final rejection preserves physical unconsumed mass for traceability');
