...
begin
  ...
  if exists(
    select 1 from (
      select value->>'plantId' as plant_id, count(*)
      from jsonb_array_elements(assignments)
      group by value->>'plantId'
      having count(*)>1
    ) duplicated
  ) then raise exception 'Una planta no puede aparecer dos veces.'; end if;
  ...
  loop
    ...
    if target_role is null or target_role not in ('operator','supervisor','technical','maintenance','admin','director') then
      raise exception 'Rol de planta inválido.';
    end if;
    if assignment ? 'active' and jsonb_typeof(assignment->'active')<>'boolean' then
      raise exception 'Estado de membresía inválido.';
    end if;
